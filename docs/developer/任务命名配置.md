...
任务目录名的长度可以通过配置文件自定义，默认为16字符，提供良好的任务可读性。
...
| 值 | 效果 | 示例 |
|----|------|------|
| `16` | 约8个汉字（推荐） | `任务-1114-023644-修复玩家死亡时背包物品未掉` |
| `12` | 约6个汉字 | `任务-1114-023644-修复玩家死亡时背包` |
| `8` | 约4个汉字 | `任务-1114-023644-修复玩家死亡` |
...
**配置: max_description_length = 16 (默认，推荐)**
```
...
**位置**: `templates/.claude/hooks/orchestrator/user_prompt_handler.py`

```python
# 生成任务ID - 保留中文，依赖UTF-8编码
...
**位置**: `templates/.claude/hooks/utils/config_loader.py`
...
DEFAULT_CONFIG = {
    "task_naming": {
        "max_description_length": 16  # 默认16字符
    }
}
```
...
| `templates/.claude/hooks/utils/config_loader.py` | 配置加载器模块 |
| `templates/.claude/hooks/orchestrator/user_prompt_handler.py` | Hook入口（任务初始化调用配置） |
...
### Q2: 如果配置文件不存在会怎样？
**A**: 使用默认值 `max_description_length = 16`，系统自动降级，不会报错。
