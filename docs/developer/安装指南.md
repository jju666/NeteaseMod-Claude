...
> **重大更新**: BUG修复 - unified-workflow-driver异常隔离与datetime导入修复
...
```bash
# 克隆项目到本地（不包含子模块，减少下载时间）
git clone https://github.com/jju666/NeteaseMod-Claude.git
cd NeteaseMod-Claude

# 可选：如果需要离线文档，手动初始化子模块
git submodule update --init --recursive
```

**注意**:
- 官方文档（MODSDK Wiki、Bedrock Wiki）作为Git Submodule托管，**首次安装可跳过**
- 工作流会自动使用在线查询（WebFetch），下载速度不受影响
- 如果需要离线文档，执行 `git submodule update --init --recursive`（需要1-3分钟）
...
**Python依赖（自动安装，无需手动操作）**:
- Python 3.6+ 标准库（json, os, sys, datetime等）
- `plyer`（可选）: 桌面通知功能，安装方法见下文
...
4. **官方文档下载**（可选）: 如果存在Git Submodule，复制到全局目录
5. **命令注册**:
   - **Windows**: 创建 `initmc.cmd` 和 `uninstallmc.cmd` 批处理脚本到用户目录
   - **Unix/Linux/Mac**: 添加别名到 `~/.bashrc` 和 `~/.zshrc`

**注意**: `npm run deploy` 和 `npm run install-global` 是同一个命令的别名。
