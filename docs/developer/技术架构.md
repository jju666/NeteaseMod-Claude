...
```mermaid
flowchart TD
    Start[开始分析] --> ScanFiles[扫描所有.py文件]
    ScanFiles --> DetectType[检测项目类型]

    DetectType --> CheckModMain{存在modMain.py?}
    CheckModMain -->|是| Standard[标准MODSDK项目]
    CheckModMain -->|否| CheckHints[检测其他特征]

    CheckHints --> |behavior_packs/| NetEaseMap[网易地图项目]
    CheckHints --> |deploy.json| Apollo[Apollo架构]

    Standard --> ScanSystems[扫描Systems]
    NetEaseMap --> ScanSystems
    Apollo --> ScanSystems

    ScanSystems --> CalcComplexity[计算复杂度]
    CalcComplexity --> InferBusiness[推断业务类型]

    InferBusiness --> GenerateReport[生成分析报告]
...
sequenceDiagram
    ...
    Init->>Gen: 传入AnalysisReport
    Gen->>Gen: _buildReplacements() 构建替换表

    Note over Gen: Layer 1 生成
    Gen->>Template: 读取mc.md.template
    Gen->>Gen: replacePlaceholders()
    Gen->>Target: 写入.claude/commands/mc.md

    Gen->>Template: 读取settings.json.template
    Gen->>Target: 写入.claude/settings.json

    Gen->>Template: 复制Hook脚本（5个.py）
    Gen->>Target: 部署到.claude/hooks/

    Note over Gen: Layer 2 生成
    Gen->>Gen: _generateSystemDoc() 遍历Systems
    Gen->>Target: 写入markdown/systems/*.md

    Gen->>Gen: _generateAdaptiveDocs() 自适应组件
    Gen->>Target: 写入markdown/components/*.md

    Note over Gen: Layer 3 生成
    Gen->>Target: 创建markdown/core/（空目录）
    Gen->>Target: 创建markdown/business/（框架）

    Note over Gen: 软连接创建
    Gen->>Target: 创建.claude/core-docs/ -> upstream
```
...
```mermaid
flowchart LR
    Start[扫描所有.py文件] --> Group[按目录分组]
    Group --> Infer[推断目录用途]

    Infer --> Signal1[信号1: 目录名称模式]
    Infer --> Signal2[信号2: 文件内容分析]
    Infer --> Signal3[信号3: 文件数量/复杂度]
    Infer --> Signal4[信号4: 配置文件特征]

    Signal1 --> Aggregate[聚合信号]
    Signal2 --> Aggregate
    Signal3 --> Aggregate
    Signal4 --> Aggregate

    Aggregate --> Score{置信度 >= 0.6?}
    Score -->|是| NeedDoc[需要文档化]
    Score -->|否| Skip[跳过]

    NeedDoc --> Classify[分类存储]
...
sequenceDiagram
    ...
    Note over Init: 步骤1: 版本检测
    Init->>Init: 检查是否需要迁移

    Note over Init: 步骤2: 项目分析
    Init->>Analyzer: analyze()
    Analyzer->>FS: 扫描.py文件
    Analyzer->>Discovery: discover()
    Discovery-->>Analyzer: 项目结构
    Analyzer-->>Init: AnalysisReport

    Note over Init: 步骤3: 文档生成
    Init->>Generator: generateAll()
    Generator->>FS: 读取templates
    Generator->>FS: 写入.claude/
    Generator->>FS: 写入markdown/
    Generator-->>Init: 完成

    Note over Init: 步骤4: 创建软连接
    Init->>FS: ln -s upstream .claude/core-docs/
    Init->>FS: ln -s upstream markdown/

    Note over Init: 步骤5: 输出报告
    Init->>User: 部署完成日志
```
...
sequenceDiagram
    ...
    User->>Claude: /mc 修复商店BUG
    Claude->>Hook: UserPromptSubmit
    Hook->>State: 初始化状态
    State-->>Hook: 状态文件创建成功

    Note over Claude: AI尝试跳过文档查阅
    Claude->>Hook: PreToolUse[Read Python文件]
    Hook->>State: 读取步骤2状态
    State-->>Hook: step2_doc_count = 0
    Hook-->>Claude: EXIT CODE 2（阻止）
    Claude->>User: "需要先阅读≥3个文档"

    Note over Claude: AI被迫读取文档
    Claude->>Hook: Read[开发规范.md]
    Hook->>State: PostToolUse追踪
    State->>State: step2_doc_count++

    Note over Claude: 读取3个文档后
    Claude->>Hook: PreToolUse[Read Python文件]
    Hook->>State: 读取步骤2状态
    State-->>Hook: step2_doc_count = 3
    Hook-->>Claude: EXIT CODE 0（允许）

    Note over Claude: 任务完成
    User->>Claude: 点击Stop按钮
    Claude->>Hook: Stop事件
    Hook->>State: 检查cleanup_completed
    State-->>Hook: False
    Hook-->>Claude: EXIT CODE 2（阻止）
    Claude->>User: "需要完成收尾工作"
```
...
```mermaid
flowchart TD
    Start[AI需要读取开发规范.md] --> Check1{覆盖层存在?}

    Check1 -->|是| CoreOverride[markdown/core/开发规范.md]
    Check1 -->|否| Check2{基线层存在?}

    Check2 -->|是| CoreBaseline[.claude/core-docs/开发规范.md]
    Check2 -->|否| Fallback[在线查询/报错]

    CoreOverride --> Version{文件版本}
    Version --> |v18.4| UseOverride[使用覆盖层内容]

    CoreBaseline --> Symlink[通过软连接读取]
    Symlink --> Upstream[上游仓库文件]

    UseOverride --> AI[返回给AI]
    Upstream --> AI
    Fallback --> AI
...
flowchart TB
    ...
    subgraph Phase1[阶段1: 工具特定处理]
        Read[Read工具<br/>文档阅读记录]
        WriteEdit[Write/Edit工具<br/>代码修改记录]
        Bash[Bash工具<br/>测试失败检测]
    end

    subgraph Phase2[阶段2: 状态更新]
        Timestamp[更新时间戳<br/>meta.updated_at]
    end

    subgraph Phase3[阶段3: 步骤检查与推进]
        StepCheck[检查步骤完成条件]
        StepAdvance[推进到下一步]
        StateSync[同步workflow-state.json]
    end

    subgraph Phase4[阶段4: 循环检测与专家触发]
        LoopDetect[检测bug修复循环]
        ExpertTrigger[触发专家分析]
    end

    subgraph Phase5[阶段5: 状态保存]
        SaveMeta[保存.task-meta.json]
    end

    Phase1 -->|独立try-catch| Phase2
    Phase2 -->|独立try-catch| Phase3
    Phase3 -->|独立try-catch| Phase4
    Phase4 -->|独立try-catch| Phase5
    Phase5 --> End[返回 continue: true]

    Read -.失败不影响.-> WriteEdit
    WriteEdit -.失败不影响.-> Bash

    StepCheck -.失败不影响.-> StepAdvance
    StepAdvance -.失败不影响.-> StateSync

    LoopDetect -.失败不影响.-> ExpertTrigger
...
graph LR
    subgraph "状态源"
        M1[.task-meta.json<br/>任务元数据]
        M2[workflow-state.json<br/>工作流状态机]
        M3[.task-active.json<br/>活跃任务标志]
    end

    subgraph "同步流程"
        S1[user-prompt-submit<br/>创建所有文件]
        S2[unified-driver<br/>双向同步]
        S3[post-archive<br/>清理活跃标志]
    end
