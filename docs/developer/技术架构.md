...
>
> **⚠️ 架构更新**: v20.0.3引入了Hooks驱动的渐进式工作流,任务状态机升级为v2.0.3格式
...
| **工作流强制** | Hook机制、状态追踪、步骤验证 | 5个Python Hook脚本 |

### 1.3 技术特色

1. **双层文档架构**：上游基线（软连接）+ 项目覆盖层（用户定制）
2. **自适应发现机制**：无需硬编码组件类型，自动推断项目结构
3. **多层Hook执行力系统**：通过5个Hook脚本实现100%工作流强制执行
4. **智能文档路由**：AI自动选择合适的文档（覆盖层优先）
5. **渐进式迁移**：支持从v15.x到v18.x的无损升级
...
graph TB
    ...
    subgraph "Hook强制执行层"
        ...
        EnforceCleanup[enforce-cleanup.py<br/>收尾强制]
...
| **workflow-state.json** | 任务执行状态文件，追踪步骤完成情况 |
