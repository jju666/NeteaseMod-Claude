...
> **适用版本**: v20.0.3+
>
> **⚠️ v20.0.3更新**: 新增SessionStart Hook数据流，任务状态机升级为v2.0.3格式
...
sequenceDiagram
    ...
    rect rgb(232, 245, 233)
        Note over CLI,Init: 第二阶段：工作流初始化
        ...
        alt 同步模式 (--sync)
            ...
            Note right of Init: 版本检测<br/>软连接更新<br/>废弃文件清理
        else 首次部署或重新部署
            Init->>Init: 检测迁移需求
    ...
    rect rgb(248, 231, 253)
        ...
        Note right of Generator: 部署 Hook 文件
        Generator->>FS: 写入命令文件
...
flowchart TD
    ...
    L --> L1[部署 settings.json]
    L --> L2[复制核心 Hook 脚本]
    L --> L3[生成模板 Hook 文件]
    L --> L4[设置可执行权限]
    L --> L5[初始化任务模式状态文件]

    L1 & L2 & L3 & L4 & L5 --> M[创建软连接]

    M --> M1[SymlinkManager.createAllSymlinks]
    M --> M2[SymlinkManager.createMarkdownSymlinks]

    M1 & M2 --> N[生成 markdown/README.md]

    N --> O[创建 markdown/core/ 目录]

    O --> P[生成 tasks/README.md]

    P --> Q[生成 workflow-manifest.json]
    Q --> Q1[记录版本号]
    Q --> Q2[计算基线哈希]
    Q --> Q3[记录安装时间]

    Q1 & Q2 & Q3 --> R[Layer 1 完成]
...
```mermaid
flowchart TD
    A[_generateLayer2] --> B[遍历 codeStructure.systems]

    B --> C{对于每个 System}
    C --> D[_detectExistingSystemDoc]

    D --> E{现有文档存在?}
    E -->|是| F[_assessDocQuality]
    E -->|否| G[生成新文档]
