...
class AdaptiveDocDiscovery {
  ...
  locateBehaviorPacks() {
    ...
    // 使用Node.js原生API递归查找（深度限制为2层）
    try {
      const result = this.findDirectoryRecursive(this.projectRoot, /^behavior_pack/i, 2);
      if (result) {
        return result;
      }
    } catch (e) {
   ...
   * 递归查找目录（原生实现）
   */
  findDirectoryRecursive(basePath, pattern, maxDepth, currentDepth = 0) {
    ...
    try {
      ...
      for (const entry of entries) {
        if (entry.isDirectory()) {
          ...
          // 递归搜索子目录
          if (currentDepth < maxDepth) {
            const found = this.findDirectoryRecursive(fullPath, pattern, maxDepth, currentDepth + 1);
            if (found) {
              return found;
            }
          }
  ...
  async scanAllClasses() {
    ...
    try {
      // 递归查找所有.py文件
      const pyFiles = this.findPythonFilesRecursive(this.behaviorPackPath);
   ...
   * 递归查找所有Python文件
   */
  findPythonFilesRecursive(basePath) {
    const pyFiles = [];
