...
function getTemplatePath(templateName, projectType = 'General') {
  ...
  } else if (templateName === 'enhance-docs.md') {
    return path.join(TEMPLATES_DIR, '.claude', 'commands', 'enhance-docs.md.template');
  } else if (templateName === 'validate-docs.md') {
    return path.join(TEMPLATES_DIR, '.claude', 'commands', 'validate-docs.md.template');
  } else if (templateName === 'discover.md') {
    return path.join(TEMPLATES_DIR, '.claude', 'commands', 'discover.md.template');
  } else if (templateName === 'review-design.md') {
    return path.join(TEMPLATES_DIR, '.claude', 'commands', 'review-design.md.template');
  } else if (templateName === 'analyze-performance.md') {
    return path.join(TEMPLATES_DIR, '.claude', 'commands', 'analyze-performance.md.template');
  } else if (templateName === 'validate-architecture.md') {
    return path.join(TEMPLATES_DIR, '.claude', 'commands', 'validate-architecture.md.template');
  } else if (templateName === 'explain-why.md') {
    return path.join(TEMPLATES_DIR, '.claude', 'commands', 'explain-why.md.template');
  } else if (templateName === 'generate-diagram.md') {
    return path.join(TEMPLATES_DIR, '.claude', 'commands', 'generate-diagram.md.template');
  } else if (['开发规范.md', '问题排查.md', '快速开始.md'].includes(templateName)) {
    return path.join(TEMPLATES_DIR, 'markdown', `${templateName}.template`);
