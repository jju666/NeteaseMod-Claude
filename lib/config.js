...
// v20.2.1修复: 简化路径解析逻辑，始终使用npm包路径
// 这确保每次 npm update 都能获得最新的模板文件

// npm包路径（lib目录的上一级）
// - 本地开发: /path/to/project/
// - npm全局安装: /usr/local/lib/node_modules/netease-mod-claude/ (Unix)
//                 C:\Users\xxx\AppData\Roaming\npm\node_modules\netease-mod-claude\ (Windows)
const NPM_PACKAGE_HOME = path.resolve(__dirname, '..');

// 遗留全局路径（仅用于迁移提示）
const LEGACY_GLOBAL_HOME = path.join(os.homedir(), '.claude-modsdk-workflow');

// v20.2.1: 始终使用npm包路径作为WORKFLOW_HOME
// 移除双路径设计，简化逻辑，确保模板始终为最新版本
...
// 版本号（v20.2.1+: 从package.json读取，确保版本一致性）
...
const PLACEHOLDERS = {
  ...
  GLOBAL_DOCS_PATH: '{{GLOBAL_DOCS_PATH}}',  // v17.4: 全局docs目录路径（~/.claude-modsdk-workflow/docs/）
...
function getTemplatePath(templateName, projectType = 'General') {
  ...
  // v18.2: Claude Code settings.json配置
  else if (templateName === 'settings.json') {
    return path.join(TEMPLATES_DIR, '.claude', 'settings.json.template');
  }
  // v18.1: Hook 模板（任务隔离与上下文恢复）
  else if (templateName === 'user-prompt-submit-hook.sh') {
    return path.join(TEMPLATES_DIR, '.claude', 'hooks', 'user-prompt-submit-hook.sh.template');
  } else if (templateName === 'edit-hook.sh') {
    return path.join(TEMPLATES_DIR, '.claude', 'hooks', 'edit-hook.sh.template');
  } else if (templateName === 'read-hook.sh') {
    return path.join(TEMPLATES_DIR, '.claude', 'hooks', 'read-hook.sh.template');
  }
  // 其他模板
