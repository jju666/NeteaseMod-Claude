...
class DocumentGenerator {
  ...
  async _generateLayer1(targetPath) {
    ...
    // 2.6. ⭐ v3.0 Final: YAML规则文件（声明式配置）
    console.log('[生成器] 部署YAML规则文件（.claude/rules/）...');
    this._deployRules(targetPath);
    console.log('[生成器] ✅ YAML规则文件部署完成');
    ...
    // 7. 创建tasks/README.md 和 context.md.template
    writeFile(
    ...
    const templateSourcePath = path.resolve(this.upstreamPath, 'templates/tasks/context.md.template');
    ...
    if (fs.existsSync(templateSourcePath)) {
      ...
      console.warn('[生成器] ⚠️ 任务上下文模板不存在，跳过部署');
    }
    ...
    // v20.2.11: 直接从package.json读取版本，避免模块缓存问题
    const upstreamVersion = versionChecker.getUpstreamVersion();
    const manifestData = {
      version: upstreamVersion,
      baselineHashes: baselineHashes,
      installedAt: new Date().toISOString()
    };
    versionChecker.writeManifest(manifestData);
...
`;
  ...
  _deployHooks(targetPath, replacements) {
    // 使用顶部导入的 fs-extra 和 path (不要重复 require)
    ...
    // ⭐ v20.0.4修复: 使用path.resolve()避免Windows中文路径Bug
    const templatesHooksDir = path.resolve(this.upstreamPath, 'templates/.claude/hooks');

    // v20.3.0改进: 支持子目录复制（如core/模块）
    // 扫描templates/目录,自动部署所有.py/.sh文件、README.md和子目录
    let allHookFiles = [];
    let allHookDirs = [];
    try {
      const entries = fs.readdirSync(templatesHooksDir, { withFileTypes: true });
      allHookFiles = entries
        .filter(f => f.isFile() && (f.name.endsWith('.py') || f.name.endsWith('.sh') || f.name === 'README.md'))
        .map(f => f.name);
      allHookDirs = entries
        .filter(f => f.isDirectory() && !f.name.startsWith('.'))
        .map(f => f.name);
      console.log(`[生成器] 发现 ${allHookFiles.length} 个hooks文件, ${allHookDirs.length} 个子目录`);
    } catch (err) {
      console.error(`[生成器] ⚠️  无法读取hooks目录: ${err.message}`);
      // v21.0: 降级方案已移除（新架构下hooks全部在子目录中）
      allHookFiles = ['README.md'];
      allHookDirs = ['core', 'lifecycle', 'orchestrator', 'validators', 'archiver', 'monitors', 'utils'];
    }

    // v20.2.6: 清理目标目录中的旧Hook文件（上游已删除的文件）
    console.log('[生成器] 清理旧Hook文件...');
    let cleanedCount = 0;
    try {
      if (fs.existsSync(hooksDir)) {
        ...
        for (const existingFile of existingHooks) {
          // 如果上游模板中不存在该文件，则删除
          if (!allHookFiles.includes(existingFile)) {
            const targetPath = path.join(hooksDir, existingFile);
      ...
      if (cleanedCount > 0) {
        console.log(`[生成器] 清理完成，删除 ${cleanedCount} 个旧文件`);
      } else {
        console.log('[生成器] 无需清理（无旧文件）');
      }
    } catch (err) {
      console.error(`[生成器] ⚠️  清理旧文件失败: ${err.message}`);
    }

    // v20.3.1: 清理下游项目中不存在于上游的子目录
    console.log('[生成器] 清理废弃的子目录...');
    let cleanedDirsCount = 0;
    try {
      if (fs.existsSync(hooksDir)) {
        const existingDirs = fs.readdirSync(hooksDir, { withFileTypes: true })
          .filter(f => f.isDirectory() && !f.name.startsWith('.'))
          .map(f => f.name);

        for (const existingDir of existingDirs) {
          // 跳过特殊目录
          if (existingDir === '__pycache__') {
            continue;
          }

          // 如果上游模板中不存在该目录，则删除
          if (!allHookDirs.includes(existingDir)) {
            const targetDirPath = path.join(hooksDir, existingDir);
            fs.removeSync(targetDirPath);
            console.log(`   🗑️  已删除废弃目录: ${existingDir}/`);
            cleanedDirsCount++;
          }
        }
      }
      if (cleanedDirsCount > 0) {
        console.log(`[生成器] 子目录清理完成，删除 ${cleanedDirsCount} 个废弃目录`);
      } else {
        console.log('[生成器] 无需清理（无废弃目录）');
      }
    } catch (err) {
      console.error(`[生成器] ⚠️  清理废弃目录失败: ${err.message}`);
    }
    ...
    for (const hookFile of allHookFiles) {
      ...
      if (fs.existsSync(sourcePath)) {
        try {
          ...
          console.log(`   ❌ 复制失败: ${hookFile} (${err.message})`);
          failedFiles.push(hookFile);
        }
      } else {
    ...
    // v20.3.0: 复制子目录（如core/模块）
    let copiedDirsCount = 0;
    for (const dirName of allHookDirs) {
      // 跳过废弃目录和Python缓存目录
      if (dirName === 'deprecated' || dirName === '__pycache__') {
        console.log(`   ⏭️  跳过目录: ${dirName}/`);
        continue;
      }

      const sourceDir = path.join(templatesHooksDir, dirName);
      const targetDir = path.join(hooksDir, dirName);

      try {
        // 递归复制整个目录
        fs.copySync(sourceDir, targetDir);
        console.log(`   ✅ 已复制目录: ${dirName}/`);
        copiedDirsCount++;
      } catch (err) {
        console.log(`   ❌ 复制目录失败: ${dirName}/ (${err.message})`);
      }
    }

    // v20.2.6: 部署验证
    console.log('\n[生成器] 部署验证...');
    console.log(`   📊 期望部署: ${allHookFiles.length} 个文件, ${allHookDirs.length} 个子目录`);
    console.log(`   ✅ 成功复制: ${copiedCount} 个文件, ${copiedDirsCount} 个子目录`);
    console.log(`   🗑️  清理旧文件: ${cleanedCount} 个`);
    console.log(`   🗑️  清理废弃目录: ${cleanedDirsCount} 个`);

    if (failedFiles.length > 0) {
      console.log(`   ❌ 失败文件: ${failedFiles.length} 个`);
      console.log(`   详细列表: ${failedFiles.join(', ')}`);
      console.error('\n[生成器] ⚠️  警告: Hook部署不完整，请检查上述失败文件');
    } else if (copiedCount === allHookFiles.length && copiedDirsCount === allHookDirs.length) {
      console.log('   ✅ 部署验证通过：所有Hook文件和子目录已成功部署');
    } else {
      console.log(`   ⚠️  警告: 复制数量与期望不匹配`);
    }

    // 3.5. 部署玩法知识库（v19.0新增）
    console.log('[生成器] 部署玩法知识库...');
    // ⭐ v20.0.4修复: 使用path.resolve()避免Windows中文路径Bug
    const kbSource = path.resolve(this.upstreamPath, '.claude/knowledge-base.json');
    const kbTarget = path.resolve(targetPath, '.claude/knowledge-base.json');
  ...
  /**
   * ⭐ v3.0 Final新增：部署 YAML 规则文件（声明式配置）
   * @param {string} targetPath - 项目路径
   */
  _deployRules(targetPath) {
    // 1. 确保 .claude/rules/ 目录存在
    const rulesDir = path.join(targetPath, '.claude', 'rules');
    ensureDir(rulesDir);

    // 2. 复制所有YAML规则文件
    const templatesRulesDir = path.resolve(this.upstreamPath, 'templates/.claude/rules');

    if (!fs.existsSync(templatesRulesDir)) {
      console.log('   ⚠️  未找到上游rules目录，跳过部署');
      return;
    }

    // 扫描所有YAML文件
    const yamlFiles = fs.readdirSync(templatesRulesDir)
      .filter(f => f.endsWith('.yaml') || f.endsWith('.yml'));

    console.log(`   📋 发现 ${yamlFiles.length} 个YAML规则文件`);

    // 复制每个文件
    for (const yamlFile of yamlFiles) {
      const sourcePath = path.join(templatesRulesDir, yamlFile);
      const targetFilePath = path.join(rulesDir, yamlFile);
      fs.copyFileSync(sourcePath, targetFilePath);
      console.log(`   ✅ 已复制: ${yamlFile}`);
    }
  }
