...
class DocumentGenerator {
  ...
  async _generateLayer1(targetPath) {
    ...
    console.log(`[生成器] DEBUG: upstreamVersion = ${upstreamVersion}, upstreamPath = ${this.upstreamPath}`);
    const manifestData = {
      version: upstreamVersion,
      baselineHashes: baselineHashes,
      installedAt: new Date().toISOString()
    };
    console.log('[生成器] DEBUG: manifestData =', JSON.stringify(manifestData));
...
`;
  ...
  _deployHooks(targetPath, replacements) {
    ...
    // v20.0.3改进: 自动发现所有hooks文件,避免手动维护列表
    // 扫描templates/目录,自动部署所有.py/.sh文件和README.md
    let allHookFiles = [];
    try {
      allHookFiles = fs.readdirSync(templatesHooksDir)
        .filter(f => f.endsWith('.py') || f.endsWith('.sh') || f === 'README.md');
      console.log(`[生成器] 发现 ${allHookFiles.length} 个hooks文件`);
    } catch (err) {
    ...
    } else if (copiedCount === allHookFiles.length) {
      console.log('   ✅ 部署验证通过：所有Hook文件已成功部署');
    } else {
      console.log(`   ⚠️  警告: 复制数量(${copiedCount})与期望(${allHookFiles.length})不匹配`);
    }
