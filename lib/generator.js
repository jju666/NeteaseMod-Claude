...
class DocumentGenerator {
  ...
  async _generateLayer1(targetPath) {
    ...
    this._generateFromTemplate('discover.md', targetPath, '.claude/commands/discover.md', replacements);
    this._generateFromTemplate('review-design.md', targetPath, '.claude/commands/review-design.md', replacements);
    this._generateFromTemplate('analyze-performance.md', targetPath, '.claude/commands/analyze-performance.md', replacements);
    this._generateFromTemplate('validate-architecture.md', targetPath, '.claude/commands/validate-architecture.md', replacements);
    this._generateFromTemplate('explain-why.md', targetPath, '.claude/commands/explain-why.md', replacements);
    this._generateFromTemplate('generate-diagram.md', targetPath, '.claude/commands/generate-diagram.md', replacements);
...
`;
  ...
  async _deployOfficialDocs(targetPath) {
    ...
    if (!workflowHome) {
      console.log('[生成器] ⚠️  无法定位工作流目录，跳过文档部署');
