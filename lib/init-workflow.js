...
async function syncWorkflow(targetPath, options = {}) {
  ...
  // 步骤3: 检测废弃文件
  console.log('🧹 检测废弃文件...\n');
  const obsoleteFiles = versionChecker.detectObsoleteFiles(check.local, check.upstream);
    ...
    if (answer !== 'n' && answer !== 'no') {
      for (const file of obsoleteFiles) {
        ...
        if (fs.existsSync(fullPath)) {
          // 备份后删除
          const backupPath = fullPath + '.obsolete';
          fs.moveSync(fullPath, backupPath);
          console.log(`   ✅ 已清理: ${file} (备份至: ${path.basename(backupPath)})`);
        }
      }
      console.log('');
    }
  ...
  if (conflicts.length > 0) {
    ...
    console.log('\n💡 建议操作:');
    console.log('   1. 查看差异: diff .claude/core-docs/[文件] markdown/core/[文件]');
    console.log('   2. 手动合并更新\n');
  } else {
