...
class IntelligentDocMaintenance {
  ...
  _inferNameBySemanticRules(info, componentName, mapping) {
    // ⚠️ 策略1和策略2已禁用 - 避免提取开发者注释导致混乱命名
    // 原因: 注释往往是操作描述("标记地图恢复是否已开始")，不适合作为文档名

    // ⭐ 移除已存在的中文后缀（避免重复添加）
    let baseName = componentName
      .replace(/(系统|状态|预设|管理器|处理器|控制器)$/,'');

    // 策略3（改进版）: 完整翻译类名中的所有关键词
    const typeSuffix = this._inferTypeSuffix(baseName, mapping);

    // 先处理组合词（BedWars, IronGolem等）
    ...
    for (const [en, zh] of Object.entries(combinedWords)) {
      if (processedName.includes(en)) {
        processedName = processedName.replace(en, zh);
      }
    }
    ...
    const classWords = processedName
      ...
      .trim()
      .split(/\s+/)
      .filter(w => !['Server', 'Client', 'Def', 'System', 'Preset', 'Manager', 'Handler', 'State', 'Controller'].includes(w));

    if (classWords.length > 0) {
      // ⭐ 关键改进: 翻译所有关键词，而不是只取第一个
      const translatedWords = classWords
        .map(word => this._translateCoreWord(word))
        .filter(w => w && w !== '');  // 过滤空字符串

      const chineseName = translatedWords.join('');
      console.log(`   [AI命名] 类名分析: ${classWords.join(' + ')} → ${chineseName}`);
      return `${chineseName}${typeSuffix}`;
    }

    // 策略4: 回退到组件类型
    console.log(`   [AI命名] 回退策略: 使用组件名 ${baseName}`);
