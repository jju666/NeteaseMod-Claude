...
class MigrationV18 {
   ...
   * @param {number} options.autoMigrateChoice - 自动迁移选项 (1/2, 默认1)
  ...
  async migrate(options = {}) {
    ...
    // 提示用户选择迁移方式
    if (!options.autoConfirm && isInteractive) {
      console.log('请选择迁移方式：');
      ...
      // 自动确认模式或非交互式环境
      if (!isInteractive) {
        console.log('ℹ️  检测到非交互式环境，使用默认选项 [1] 保留现有CLAUDE.md\n');
      }

      const choice = options.autoMigrateChoice || 1;
