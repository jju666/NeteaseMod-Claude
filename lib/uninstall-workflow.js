...
class WorkflowUninstaller {
  constructor(projectPath) {
    ...
    // v18.0: 支持新旧版本文件名（向后兼容）
    this.manifestPath = path.join(projectPath, '.claude', 'workflow-manifest.json');
    this.versionFilePath = path.join(projectPath, '.claude', 'workflow-version.json'); // v15.x 旧格式
    ...
    this.workflowFiles = [
      ...
      '.claude/commands/mc-perf.md',
      '.claude/commands/mc-docs.md',
      '.claude/commands/mc-why.md',
      '.claude/commands/mc-discover.md',

      // 核心文档（v16.0+: 软连接或副本）
      '.claude/core-docs',

      // 版本追踪（v18.0: 支持新旧格式）
      '.claude/workflow-manifest.json',  // v16.0+ 新格式
      '.claude/workflow-version.json',   // v15.x 旧格式（兼容）
  ...
  /**
   * 检查是否已部署工作流（v18.0: 兼容新旧格式）
   */
  isWorkflowInstalled() {
    return fs.existsSync(this.manifestPath) || fs.existsSync(this.versionFilePath);
  }

  /**
   * 获取已安装的工作流版本（v18.0: 兼容新旧格式）
   */
  getInstalledVersion() {
    ...
    try {
      ...
      if (fs.existsSync(this.manifestPath)) {
        const manifestData = JSON.parse(fs.readFileSync(this.manifestPath, 'utf-8'));
      ...
      if (fs.existsSync(this.versionFilePath)) {
        ...
        return versionData.version || versionData.workflow_version;
      }
  ...
  async uninstall(options = {}) {
    ...
    if (!this.isWorkflowInstalled()) {
      ...
      console.log('\n提示: 如果您确认已部署工作流，请检查以下文件是否存在:');
      console.log('   - .claude/workflow-manifest.json (v16.0+)');
      console.log('   - .claude/workflow-version.json (v15.x)\n');
