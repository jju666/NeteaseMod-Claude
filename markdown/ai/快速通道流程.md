...
```python
# 1. 使用Glob快速定位文件
Glob(pattern="**/game_config.py")

# 2. 使用Grep定位具体行号
Grep(pattern="PLAYER_MAX_HEALTH", path="game_config.py", output_mode="content", -n=True)

# 3. 读取相关代码确认
Read(file_path="path/to/game_config.py", offset=10, limit=20)
```

**输出示例**：
```
📍 定位结果:
- 文件: behavior_packs/.../game_config.py
- 行号: 15
- 当前值: PLAYER_MAX_HEALTH = 20
- 目标值: PLAYER_MAX_HEALTH = 40
```
...
```python
Edit(
    file_path="path/to/game_config.py",
    old_string="PLAYER_MAX_HEALTH = 20",
    new_string="PLAYER_MAX_HEALTH = 40  # 调整玩家生命值平衡性（2025-XX-XX）"
)
```
...
```python
# ✅ 好的注释
PLAYER_MAX_HEALTH = 40  # 调整玩家生命值平衡性（2025-01-15）
use_new_damage_calc = True  # 修复伤害计算错误（2025-01-15）

# ❌ 不好的注释
PLAYER_MAX_HEALTH = 40  # 修改
