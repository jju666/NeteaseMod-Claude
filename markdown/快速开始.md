...
### 示例1：修复BUG
```bash
/cc 修复商店预设点击无响应，日志显示GetComponent返回None
```
...
2. 查阅文档：学习商店系统设计原则
3. 设计方案：基于文档原则设计实现
4. 分阶段实施：修改配置→修改UI→测试验证

### 示例3：性能优化
```bash
/cc 优化资源点刷新逻辑，减少tick中的计算开销
```
...
在修改某个System前，先查阅其文档：
```bash
Read(file_path="markdown/systems/ShopSystem.md")
```
