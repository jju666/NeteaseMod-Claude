...
```bash
# 使用/mc命令
/mc 创建一个ServerSystem，监听玩家加入游戏事件并发送欢迎消息
```
...
```bash
# 修复BUG
/mc 修复System初始化错误，GetComponent返回None

# 添加功能
/mc 添加玩家死亡事件监听，在聊天栏显示死亡原因

# 双端通信
/mc 创建客户端UI按钮，点击时向服务端发送事件

# EventData调试
/mc 修复EventData序列化错误，tuple改为list
...
# 根据搜索结果执行任务
/mc 创建一个2000x2000的AOI感应区，检测玩家进入
```
...
```bash
# 完整双端功能开发
/mc 创建商店系统：客户端UI按钮→服务端购买逻辑→客户端显示结果
