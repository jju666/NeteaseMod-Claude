...
def main():
    try:
        ...
        # 【v22.4新增】防止无限循环：如果Stop Hook已触发过，不再重复阻止
        if stop_hook_active:
            sys.stderr.write("[Stop Hook v22.4] stop_hook_active=true，跳过重复阻止\n")
            sys.exit(0)  # 放行
        ...
        # 🔥 [v26.0修改] 移除专家审查强制阻止逻辑
        # v26.0采用"单次审查模式"：每轮Planning只执行1次审查，结果仅供参考，不强制阻止
        # 原v25.0的强制审查循环已被移除，改由PreToolUse Hook防止重复审查

        if current_step == 'planning' and not planning.get('user_confirmed', False):
            # 🔥 [v25.1.1修复] Planning阶段前置条件检查
            # 问题根因: v26.0移除专家审查强制阻止后，Stop Hook只检查user_confirmed
            #           导致回退到Planning时，AI询问用户就被错误阻止（用户输入"D"场景）
            # 修复方案: 增加前置条件检查，只有AI完成必要步骤后才阻止
            #           - BUG修复任务: 必须完成专家审查(expert_review_completed=True)
            #           - 其他任务: 必须完成文档阅读(docs_read >= required_doc_count)
            # 收益: 解决Implementation→Planning回退后AI询问被中止的问题
            ...
            # 检查Planning阶段必要前置步骤是否完成
            if task_type == 'bug_fix':
                # BUG修复任务: 必须完成专家审查
                if not expert_review_completed:
                    # 专家审查未完成，AI还在分析问题/等待审查/询问用户，放行
                    sys.stderr.write("[Stop Hook v25.1.1] Planning前置步骤未完成(专家审查)，放行\n")
                    sys.exit(0)
            else:
                # 其他任务: 必须完成文档阅读
                ...
                if docs_read < required_docs:
                    # 文档阅读未完成，AI还在阅读分析/询问用户，放行
                    sys.stderr.write(
                        "[Stop Hook v25.1.1] Planning前置步骤未完成(文档{}/{}),放行\n".format(
                            docs_read, required_docs
                        )
                    )
                    sys.exit(0)
            ...
            # 🔥 v26.0修改：使用新的数据结构
            last_expert_review = planning.get('last_expert_review', {})
            planning_round = planning.get('planning_round', 1)

            # ✅ v27.3修复：删除Planning阶段的专家审查检查（PreToolUse已100%拦截TodoWrite）
            # 原因：
            # 1. PreToolUse的TodoWrite拦截已经100%阻止AI展示方案（无例外）
            # 2. Stop Hook的检查时机太晚（AI已向用户展示方案），体验差
            # 3. 简化逻辑，减少冗余检查和维护成本

            # 保留Planning阶段的user_confirmed检查
...
**任务ID**: {}
**当前阶段**: Planning (方案制定 - 第{}轮)

""".format(task_id[:30], planning_round)

            # 添加方案描述
            solution_summary = planning.get('solution_summary', '')
            if solution_summary:
                detail_message += u"📋 **方案摘要**:\n{}\n\n".format(solution_summary)

            # 🔥 v26.0修改：添加专家审查结果（使用新数据结构）
            if last_expert_review:
                approved = last_expert_review.get('approved', False)
                review_score = last_expert_review.get('score', 0)

                if approved:
                    detail_message += u"✅ **专家审查**: 通过\n"
                    if review_score:
                        detail_message += u"**审核评分**: {}/10\n\n".format(review_score)
                    detail_message += u"🎉 方案已通过专家审查！\n\n"
                else:
                    detail_message += u"⚠️ **专家审查**: 发现问题\n\n"
                    issues = last_expert_review.get('issues', [])
                    if issues:
                        detail_message += u"🔍 **发现的问题**:\n"
                        for idx, issue in enumerate(issues, 1):
                            detail_message += u"  {}. {}\n".format(idx, issue)
                        detail_message += u"\n"

                    suggestions = last_expert_review.get('suggestions', [])
                    if suggestions:
                        detail_message += u"💡 **改进建议**:\n"
                        for idx, suggestion in enumerate(suggestions, 1):
                            detail_message += u"  {}. {}\n".format(idx, suggestion)
                        detail_message += u"\n"

            # v30.1优化：统一ABC选项格式，与dashboard_generator保持一致
            detail_message += u"""**下一步 - 请选择反馈方式**:

📋 **反馈指南（请选择或用自然语言）**:
  A. ✅ 同意方案 → 输入 "同意"、"可以开始"、"确认" 或直接输入 "A"
  B. 🔄 需要调整 → 输入 "需要调整"、"有些问题"、"建议..." 或直接输入 "B"
  C. 🔄 重新开始 → 输入 "重来"、"完全不对"、"重新规划" 或直接输入 "C"
...
"""
            ...
            try:
                ...
                sys.stderr.write(u"[WARN v28.0] Stop Hook仪表盘生成失败: {}\n".format(e))
...
反馈示例（与仪表盘一致）:
  A. ✅ "修复了" / "修复成功" → 进入Finalization
  B. ⚠️ "基本正确，但..." / "部分成功" → 继续Implementation
  C. ❌ "没修复" / "修复失败" → 继续Implementation
  D. 🔄 "方案错误" / "需要调整" → 回滚到Planning重新分析

💡 提示：可直接输入选项字母（如"D"）或标签（如"方案错误"）或自然语言
