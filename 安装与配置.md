...
**该命令将：**
1. **AI自动发现项目组件** - 扫描项目中的所有Systems、States、Presets、Managers等组件
2. **智能推断规范化文档名** - AI阅读源代码，推断准确的中文文档名
   - 示例：`ShopServerSystem.py` → "商店购买系统.md"
   - 示例：`WaitingState.py` → "等待阶段状态.md"
3. **生成文档待补充清单** - 供后续 `/enhance-docs` 使用
4. **（可选）创建文档占位符** - 询问是否创建占位文档
...
2. `/validate-docs` → AI主动发现+规范化文档结构
3. `/enhance-docs` → AI批量生成高质量文档内容
4. `/cc` → 开发时被动维护文档

**工作流特点**：
- **最小化部署** - initmc只部署核心工作流，无无用占位符
- **AI驱动发现** - validate-docs自动发现组件并规范化命名
