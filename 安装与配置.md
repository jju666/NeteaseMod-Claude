...
| 工具 | 用途 | 命令 |
|------|------|------|
| **indexer.js** | 构建文档索引 | `node lib/indexer.js` |
| **analyzer.js** | 分析项目结构 | `node lib/analyzer.js` |
...
可以随时运行 `node lib/indexer.js` 更新索引。
...
### ✅ 项目验证
- [ ] 项目已克隆到本地
- [ ] 目录结构完整（包含CLAUDE.md、markdown/、lib/等）
- [ ] 可以在VSCode中打开项目

### ✅ 工具验证
- [ ] `npm run install-global` 执行成功
- [ ] `node lib/indexer.js` 生成 docs-index.json
- [ ] docs-index.json 文件存在于项目根目录
...
# 解决: 使用管理员权限
# Windows: 以管理员身份运行终端
# Linux/Mac: sudo npm run install-global
```
...
**检查**:
1. 是否违反CRITICAL规范（见[开发规范.md](./markdown/开发规范.md)）
2. 是否缺少必要的文件/目录
3. Python环境是否配置正确
4. 是否已构建文档索引
...
### 文档资源
1. **快速上手**: [快速开始.md](./markdown/快速开始.md)
2. **指令参考**: [Claude指令参考.md](./Claude指令参考.md)
3. **开发规范**: [开发规范.md](./markdown/开发规范.md)
4. **问题排查**: [问题排查.md](./markdown/问题排查.md)
...
3. **[开发规范.md](./markdown/开发规范.md)** - 学习CRITICAL规范
4. **执行第一个任务** - 实践中学习
